...
function Voting({ voteDiv, mainDiv, mainTitle, title, finalState }) {
    ...
    const getState = async () => {
        ...
        const response = await axios.get('http://localhost:3131/state', { headers }).then((res) => {
    ...
    const save = async (e) => {
        ...
        const token = Cookies.get('userData')
        const headers = { 'token': token };
        const response = await axios.post('http://localhost:3131/voted', party, { headers }).then(() => {
            ...
            const response = axios.get('http://localhost:3131/state', { headers }).then((res) => {
            ...
            if (isVoted == true) {
                const mail = async () => {
                    document.getElementById('mainDiv').style.display = 'none'
                    document.getElementById('afterVote').style.display = 'block'
                    const mail = await axios.post('http://localhost:3131/mail',
