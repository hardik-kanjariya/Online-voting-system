...
function AdminPage() {
  ...
  const save2 = async (e) => {
    ...
    const response = await axios.post('http://localhost:3131/deleteParty', user2, { headers }).then(res => {
      setTimeout(() => {
  ...
  const save = async (e) => {
    ...
    console.log(token);
    const headers = { 'token': token };
    const response = await axios.post('http://localhost:3131/addParty', user, { headers }).then(res => {
      setTimeout(() => {
